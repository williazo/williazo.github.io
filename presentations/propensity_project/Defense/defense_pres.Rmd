...
src="https://streamable.com/m/julio-urias-guides-l-a-to-ws-win"
...
1. Methods extended to categorical, or multiple, treatments by `r Citet(myBib,"imbens_dose_resp")`
    - Term "generalized propensity score" (GPS) is introduced <sup>1</sup>
...
.footnote[
[1] Throughout we use GPS to refer to continuous rather than categorical treatment
]
---
...
- All methodological development have focused on single univariate continuous exposure
...
# Motivating Example
---
# Project Goal<sup>1</sup>
- High obesity rates of low-income pre-school aged children in Los Angeles County `r Citep(myBib, "phfewic")`
    - 20% classified as obese during period from 2003-2009

- Estimate causal effects of non-random obesity interventions on WIC preschool-aged child obesity rates
...
.footnote[
[1] Early Childhood Obesity Systems Science (ECOSyS) study funded by NIH
...
.small[
- Source:
    - WIC administrative data<sup>1</sup>
    - Pulled from 2007-2016
- Quantification:
    - Aggregated obesity prevalence at census tracts
...
.footnote[
[1] Part of the [WIC Data Mining Project](https://lawicdata.org)
]
---
...
- Classified strategies into one of 9 categories
    - Grouped as **micro** (targeted individuals) or **macro** (global population)
...
- Catchment area construction:
    ...
    1. Form a circle with radius such that X% of clients attend the clinic within the circle
        - Macro strategies: 80%
        - Micro strategies: 50%
    1. Census tracts that overlap circle receive program macro and micro dose
]
...
- $\mathbf{D}$: multivariate exposure of dimension $m$<sup>1</sup>
...
.footnote[
[1] In simulation and application we fix *m*=2 for bivariate exposure
]
...
- Compare mvGPS to other univariate<sup>1</sup> alternative methods
    - Covariate balancing generalized propensity score, CBGPS, `r Citep(myBib,"fong2018")`
...
.footnote[
[1] Estimated twice, once for each exposure
]
...
- Three balance metrics were assessed
    1. Average weighted absolute Pearson exposure-covariate correlation
    1. Maximum weighted absolute Pearson exposure-covariate correlation
...
class: center, middle, inverse
# Application Results
## Measuring Effect of WIC Intervention
---
...
1. Overweight or Obesity Prevalence

NETS
1. Healthy Outlets per sq. mile
1. Unhealthy Outlets per sq. mile
]
---
...
- ESS for all methods including mvGPS are reduced but power is still reasonable high
...
- [mvGPS](https://cran.r-project.org/package=mvGPS) package available now on CRAN
